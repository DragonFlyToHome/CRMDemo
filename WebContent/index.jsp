		...
		$.post("${pageContext.request.contextPath}/order/showOrdersByPage",
				...
				function(data){
					...
					for (var i = 0; i < pageContent.length; i++) {
						...
						+"</td><td>"+ pageContent[i].address
						+"</td><td>"+ pageContent[i].total_money
						+"</td><td>"+ pageContent[i].customer_name
						+"</td><td><a href='javascript:deleteOrder("+ pageContent[i].order_id
								+")'>删除</a>&nbsp;&nbsp;"
						+"<a href='javascript:showOrderDetail("+ pageContent[i].order_id
						+");' "
...
<form id="myForm" action="${pageContext.request.contextPath}/customer/showCustomerByPage" method="post">
	...
	<table class="table table-hover table-bordered bg">
		...
		<c:forEach items="${usersPage.pageContent}" var="pc" varStatus="vs">
			<tr>
			<input type="hidden" name="customers[${vs.index}].customer_id" value="${pc.customer_id}"/>
				<!-- 序号：vs.index起始值为0 -->
				<td>${(usersPage.pageNo-1)*usersPage.pageSize+vs.index+1}</td>
				<td><img src=""D:/javawps/CRMDemo/WebContent/pics/${pc.self_pai_pic}"
					class="img-circle"></td>
				<td><input type="text" name="customers[${vs.index}].customer_name" value="${pc.customer_name}"/></td>
				<td><input type="text" name="customers[${vs.index}].phone_num" value="${pc.phone_num}"/></td>
				<td>
				...
				<a id="orderDetail" href="javascript:void(0);"
		...
		<td><a href="javascript:updateCustomer();" 
